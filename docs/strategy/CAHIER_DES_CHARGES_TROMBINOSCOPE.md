...
> Statut : v1 — **développement fonctionnel complet (L1-L7)** sur la branche `feature/crewing-monthly-yearbook` (cadrage validé par le service Armement le 2026-07-20 ; consentement photo et politique de rétention pris en charge par l'Armement). Déclenchement automatique par scheduler interne (APScheduler) — **aucun flux Power Automate à configurer**, cf. `docs/operations/07-trombinoscope-generation-runbook.md`. Reste L8 (tests d'intégration bout-en-bout + validation CI) avant Pull Request.
...
### Module TRB-3 — Génération manuelle & téléchargement

#### A — Vision développeur

- Routes (`app/routers/crew_router.py`) :
  - `POST /crew/trombinoscope.pdf` — régénère avec les données les plus fraîches, `Response(content=pdf_bytes, media_type="application/pdf", headers={"Content-Disposition": "attachment; filename=..."})`. **POST** (pas GET) : archive une ligne `generated_reports`, donc a un effet de bord — protégé par le token CSRF du formulaire (sécurité, review 2026-07-20 : un GET avec effet de bord contourne structurellement `CSRFMiddleware`, qui ne valide jamais GET/HEAD/OPTIONS).
  - `GET /crew/trombinoscope/latest.pdf` — **nouveau (review 2026-07-20)**, télécharge le dernier PDF déjà archivé (`report_archive.latest_report`) **sans régénérer** — lecture seule, donc GET légitime. C'est la cible du lien de la notification (module TRB-5) : l'Armement clique, télécharge, puis diffuse (ex. par email) à l'ensemble de la société.
- Permission : `require_permission("crew", "C")` sur les deux routes.
- La régénération manuelle (`POST .../trombinoscope.pdf`) enregistre une ligne `generated_reports` (`generated_by` = utilisateur courant) mais **ne déclenche pas de notification** (review 2026-07-20 — cf. module TRB-5) : l'utilisateur qui régénère est déjà informé, seule la génération mensuelle notifie. L'archivage est enveloppé dans un `try/except` : un échec d'archivage ne doit jamais priver l'utilisateur du PDF déjà rendu en mémoire.

#### B — Vision chef de projet

- Besoin : pouvoir régénérer le trombinoscope à tout moment (nouvel arrivant, erreur de photo corrigée), sans attendre la fin du mois ; et pouvoir télécharger le dernier trombinoscope généré (mensuel ou manuel) sans en produire un nouveau.
- Règles : la régénération manuelle utilise toujours les données les plus fraîches disponibles au moment de la demande (pas de cache figé sur plusieurs jours) ; elle ne notifie personne (l'auteur de l'action le sait déjà).
- Acceptance : depuis `/crew`, un utilisateur `armement` régénère et reçoit immédiatement le PDF à jour, sans notification ; depuis la même page (ou depuis la notification mensuelle), il télécharge le dernier trombinoscope archivé sans en régénérer un nouveau.

### Module TRB-4 — Génération automatique (fin de mois)

#### A — Vision développeur

- **Décision révisée (2026-07-20, cf. §11)** : déclenchement par **scheduler interne** (`app/services/trombinoscope_scheduler.py`, APScheduler, `CronTrigger(day="last", hour=23, minute=55)`, timezone Europe/Paris) plutôt que par un cron externe Power Automate — le trombinoscope n'appelle aucun service tiers, contrairement à Marad/météo/veille, donc pas besoin d'un flux externe pour cette seule fonctionnalité. Démarré/arrêté via `@app.on_event("startup"/"shutdown")`, toggle `TROMBINOSCOPE_SCHEDULER_ENABLED` (défaut `true`).
- **Garde-fou multi-workers** (l'app tourne avec 2 workers uvicorn) : verrou consultatif transactionnel `pg_try_advisory_xact_lock` (libéré automatiquement au commit/rollback, pas de gestion manuelle) — seul le worker qui l'obtient exécute la génération. Vérification complémentaire sur `generated_reports` (période déjà générée automatiquement ?) pour couvrir un redémarrage le même jour.
- Un endpoint manuel de secours est conservé : `POST /api/trombinoscope/generate`, protégé par `X-API-Token: <TROMBINOSCOPE_API_TOKEN>` (`503` si non configuré, exempté de CSRF), utile pour forcer une génération sans attendre le scheduler (tests, incident) — plus le mécanisme principal.
- `generated_by = NULL` dans `generated_reports` pour toute génération automatique (scheduler ou endpoint de secours).
- Runbook opérationnel : `docs/operations/07-trombinoscope-generation-runbook.md`.

#### B — Vision chef de projet

- Besoin : le trombinoscope du mois est disponible sans action humaine, chaque dernier jour du mois, **sans dépendre d'un flux Power Automate à configurer côté IT**, **indépendamment de toute régénération manuelle survenue plus tôt dans le mois** (confirmé par l'Armement le 2026-07-20 — la génération mensuelle doit toujours avoir lieu, que l'Armement ait ou non régénéré manuellement entre-temps).
- Règles : si la génération automatique échoue (ex. erreur transitoire), elle doit être rejouable manuellement sans perte (le module TRB-3 et l'endpoint de secours servent de filet de sécurité). Le garde-fou anti-doublon (§A) ne compare qu'aux générations **automatiques** de la période (`generated_by IS NULL`) — une régénération manuelle dans le mois n'empêche jamais la génération automatique de fin de mois, par choix assumé.
- Acceptance : au dernier jour d'un mois simulé en environnement de test, le scheduler produit une nouvelle ligne `generated_reports` et une notification, sans intervention humaine ; un redémarrage de l'application le même jour ne produit pas de doublon ; une régénération manuelle survenue plus tôt dans le mois n'empêche pas la génération automatique de fin de mois.
...
- **Révisé (review 2026-07-20)** : `notifications.notify_trombinoscope_generated()` n'est appelée que par la génération **automatique** (scheduler `_generate_job`, et l'endpoint de secours `POST /api/trombinoscope/generate`) — **pas** par la régénération manuelle (module TRB-3). `link="/crew/trombinoscope/latest.pdf"` (route de téléchargement en lecture seule, pas la route de régénération qui casserait en 405 au clic depuis un `<a href>`). Réutilise le mécanisme `target_role` existant, pas de nouvelle table de destinataires en v1.
...
- Besoin : le service Armement est informé dès que le trombinoscope **mensuel** est disponible, sans avoir à vérifier manuellement ; il télécharge alors le PDF et le diffuse lui-même (ex. par email) à l'ensemble de la société — l'application ne gère pas cette diffusion finale.
- Règles : une régénération manuelle (nouvel arrivant, correction) ne notifie personne — l'utilisateur qui régénère est déjà informé de sa propre action ; seule la génération mensuelle notifie. Les destinataires exacts (au-delà du rôle `armement`) seront définis ultérieurement par le métier — le système ne doit pas bloquer sur cette décision et doit permettre de l'ajuster sans redéveloppement.
...
L5 | Déclenchement automatique fin de mois | L2, L3 | ✅ **révisé le 2026-07-20** : scheduler interne APScheduler (`app/services/trombinoscope_scheduler.py`) plutôt que Power Automate — aucun flux externe à créer. Endpoint `POST /api/trombinoscope/generate` conservé comme déclencheur manuel de secours.
...
Taxonomie des fonctions | **Figé le 2026-07-17, validé par l'Armement le 2026-07-20** : `CrewMember.role` conserve ses valeurs canoniques françaises existantes (`CREW_ROLES`) ; le trombinoscope affiche des libellés anglais via un dictionnaire dédié dans `crew_directory.py`, séparé de `ROLE_LABELS`/`REQUIRED_ROLES` (inchangés, réservés au contrôle d'armement réglementaire). 3 nouvelles valeurs ajoutées à `CREW_ROLES` : `electricien` (Assisting Electrical Engineering Officer), `ajusteur` (Fitter), `matelot_cuisinier` (Able Seaman Cook). Mapping complet dans le module TRB-1 ci-dessus.
Consentement photo & rétention des archives | **Confirmé le 2026-07-20** : pris en charge directement par le service Armement (hors périmètre technique de ce cahier des charges) — cf. §12, questions 4 et 5 closes.
Déclenchement automatique | **Révisé le 2026-07-20** : scheduler interne (APScheduler) plutôt que cron externe Power Automate — cf. module TRB-4. Motif : le trombinoscope n'appelle aucun service tiers, donc pas de raison de dépendre d'un flux externe pour cette seule fonctionnalité (contrairement à Marad/météo/veille qui, eux, appellent réellement une API tierce). Garde-fou multi-workers via verrou Postgres transactionnel. Endpoint token-protégé conservé comme déclencheur manuel de secours.
Indépendance génération auto / manuelle | **Confirmé le 2026-07-20** (revue de code) : la génération automatique de fin de mois doit **toujours** avoir lieu, qu'une régénération manuelle ait eu lieu ou non plus tôt dans le mois — pas de logique "sauter le mois si déjà généré manuellement". Le garde-fou anti-doublon du scheduler ne compare qu'aux générations automatiques (`generated_by IS NULL`), ce qui matérialise déjà cette règle sans changement de code.
Notifications — auto uniquement | **Révisé le 2026-07-20** (revue de code) : seule la génération **automatique** (scheduler + endpoint de secours) notifie le rôle `armement`. La régénération manuelle n'envoie plus de notification (l'utilisateur qui régénère est déjà informé). Nouvelle route `GET /crew/trombinoscope/latest.pdf` (téléchargement du dernier PDF archivé, sans régénération) : c'est elle que cible le lien de la notification, et un lien/bouton dédié a été ajouté sur `/crew`. Workflow visé : notification mensuelle → l'Armement télécharge → diffuse elle-même (ex. email) à l'ensemble de la société, hors périmètre applicatif.
...
2 | ~~Le document réel utilise des intitulés de fonction en anglais maritime qui ne correspondent que partiellement à `CREW_ROLES`/`ROLE_LABELS`...~~ | **Résolu (2026-07-20)** — mapping validé par le service Armement, cf. module TRB-1 et §11. 3 nouvelles valeurs ajoutées à `CREW_ROLES` (`electricien`, `ajusteur`, `matelot_cuisinier`).
...
4 | ~~Un consentement explicite des marins est-il requis avant diffusion de leur photo...~~ | **Résolu (2026-07-20)** — pris en charge directement par le service Armement, hors périmètre technique.
5 | ~~Quelle politique de rétention pour l'archive des trombinoscopes générés...~~ | **Résolu (2026-07-20)** — pris en charge directement par le service Armement, hors périmètre technique. Techniquement, aucune purge automatique n'est implémentée en v1 (archivage illimité par défaut) ; à ajuster si l'Armement définit une politique de rétention précise.
6 | Le document réel analysé ne comporte pas de date/mois de génération visible — faut-il ajouter cette mention (recommandé) sur la version automatisée ? | En attente — **non implémenté** dans le gabarit actuel (le mois/année n'apparaît que dans les métadonnées PDF cachées, pas visible sur les pages), à trancher avant mise en production
