...
**`client_validated`** | **Le client**, depuis le portail `/p/{token}` | ⚠️ Oui, mais toute modification **repasse l'état à `draft`** et invalide la validation | PDF draft, mention « validé par le client le … » |
...
`bl_client_validated_by` | `String(200)` | Contact expéditeur (pas un compte : accès par token) |
...
2. **Qui valide côté client ?** Le portail `/p/{token}` est l'accès de
   l'**expéditeur**, qui n'est pas toujours le client contractuel (constat
   d'audit : deux fils de messagerie séparés pour cette raison). La validation
   du draft relève-t-elle de l'expéditeur qui remplit la packing list, ou du
   client titulaire du booking (espace `/me`) ? **Cela change l'écran et la
   permission.**
