...
4 | **Une migration vide et anonyme est apparue — origine inconnue** (RAF R8) | `4f4eeb7bfc89_.py` (« empty message », non suivie par git) était posée sur la migration de fusion et en devenait la tête. Écartée. ⚠️ **Je l'avais d'abord attribuée à un hook du harnais : c'était une inférence fausse.** Recherche exhaustive le 2026-08-03 : aucun hook configuré (projet, local, global, git, VS Code) et rien dans le dépôt n'appelle `alembic revision`. **L'origine reste inconnue** — donc la seule consigne solide est de **vérifier la tête unique avant de créer une migration** |
