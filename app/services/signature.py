...
def compute_noon_hash(n) -> str:
    parts = [
        ...
        _norm(n.sog_avg),
        _norm(n.cog_avg),
        _norm(n.wind_speed_kn),
        _norm(n.wind_direction_deg),
        _norm(n.distance_24h_nm),
        _norm(n.rob_fuel_l),
        _norm(n.fuel_consumed_24h_l),
        _norm(n.remarks),
        _norm(n.signed_by_id),
        _norm(n.signed_at),
    ]
    return hashlib.sha256("|".join(parts).encode("utf-8")).hexdigest()
...
def compute_watch_hash(w) -> str:
    parts = [
        _norm(w.leg_id),
        _norm(w.watch_date),
        _norm(w.watch_period),
        _norm(w.officer_on_watch),
        _norm(w.entry),
        _norm(w.weather_summary),
        _norm(w.signed_by_id),
        _norm(w.signed_at),
    ]
    return hashlib.sha256("|".join(parts).encode("utf-8")).hexdigest()
...
def sign_record(record, user, *, hash_fn) -> None:
    ...
    record.signed_by_name = getattr(user, "full_name", None) or getattr(user, "username", "") or "—"
