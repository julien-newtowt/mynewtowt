...
async def diagnose() -> dict:
    ...
    cible) pour distinguer « connecté mais compte vide » de « connecté avec
    données ».

    Renvoie ``{configured, reachable, authenticated, classification, base_url,
    working_strategy, attempts, vessels_count, crew_count}``
    (``crew_count`` = None si non vérifiable, 0 si vide, N sinon).
    """
    global _working_strategy
    if not enabled():
        return {
