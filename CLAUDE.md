...
| RH (SIRH) | `/rh` | 🟡 congés marins + collaborateurs sédentaires (dossier, CRUD, import fichier, contrats & avenants + alertes d'échéance, congés/absences + self-service `/rh/moi`, EVP/paie + verrouillage de période, export Silae CSV + journal des lots) — SIRH en cours (cf. `docs/strategy/CAHIER_DES_CHARGES_SIRH.md`) |
