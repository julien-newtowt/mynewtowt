...
async def test_r20_ballast_voyage_out_of_scope(db):
    vessel, leg = await _r20_leg(
        db, cargo_bl=Decimal("900"), cargo_mrv=Decimal("0"), condition="ballast"
    )
...
async def test_r21_declared_vs_real_duration_exact_boundary(db, declared_h, result):
    ...
    cur = SimpleNamespace(
        event_type="noon", datetime_utc=T0 + timedelta(hours=24), time_from_last_report_h=declared_h
    )
...
async def test_r21_derives_from_sosp_cumulative(db):
    ...
    prev = SimpleNamespace(event_type="noon", datetime_utc=T0, time_from_sosp_h=Decimal("10"))
    cur = SimpleNamespace(
        event_type="noon", datetime_utc=T0 + timedelta(hours=24), time_from_sosp_h=Decimal("40")
    )  # Δ déclaré = 30 vs réel 24
    out = await _run(db, "R21", [prev, cur], 1)
...
async def test_r22_carbon_vs_noon_exact_boundary(db, noon_consos, result):
    leg, carbon = await _r22_reports(db, carbon_total=Decimal("10"), noon_consos=noon_consos)
...
async def _r23_bunker(db, *, mass_t, alloc_volume, alloc_density, capacity=None):
    ...
    db.add(
        BunkerTankAllocation(
            bunker_id=bunker.id, tank_id=tank.id, volume_m3=alloc_volume, density_t_m3=alloc_density
        )
    )
    await db.flush()
...
async def test_r23_mass_vs_allocations_exact_boundary(db, alloc_volume, result):
    vessel, bunker = await _r23_bunker(
        db, mass_t=Decimal("20"), alloc_volume=alloc_volume, alloc_density=Decimal("1.0")
    )
...
async def _reading(
    db, vessel, *, at, action="measurement", total=Decimal("30"), rob=Decimal("30"), compartments=()
):
...
async def test_r26_chained_voyages_pass(db):
    vessel = await _vessel(db)
    leg1 = await _leg(
        db, vessel, code="1AFRBR6", dep="FRFEC", arr="BRBEL", dep_c="FR", arr_c="BR", etd=T0
    )
    await _leg(
        db,
        vessel,
