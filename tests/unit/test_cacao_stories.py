...
def test_erp_fields_are_injected() -> None:
    ...
    # 🔴 Le tonnage n'est plus injecté : les récits ne chiffrent plus
    # d'évitement (base écartée, méthodologie v3.0 §11.1). Ce qui est
    # vérifié ici reste l'essentiel — région, producteur et navire viennent
...
def test_short_no_longer_carries_a_kg_number() -> None:
    ...
    vérifiable — et non une comparaison à un transport conventionnel dont la
    base est écartée (méthodologie v3.0 §11.1).
    """
    txt = cs.render_story("equateur", "en", "short", co2_kg=250)
...
def test_thousands_separator_per_language() -> None:
    ...
    Vérifie désormais qu'AUCUN tonnage ne revient, quelle que soit la valeur
    passée (``co2_kg`` reste accepté par la signature mais est ignoré).
    """
    for lang in ("fr", "en", "pt-br"):
        txt = cs.render_story("equateur", lang, "long", co2_kg=1200)
