...
def _ev(leg_id, t, **kw):
    return MRVEvent(leg_id=leg_id, event_kind=kw.pop("kind", "noon_consumption"),
                    recorded_at=t, fuel_type="MDO", **kw)
...
async def test_recompute_me_ae_rob_from_counters(db):
    ...
    db.add(_ev(1, t0, port_me_do_counter=Decimal(100), stbd_me_do_counter=Decimal(100),
               fwd_gen_do_counter=Decimal(50), aft_gen_do_counter=Decimal(50)))
    db.add(_ev(1, t0 + timedelta(days=1),
               port_me_do_counter=Decimal(105), stbd_me_do_counter=Decimal(105),
               fwd_gen_do_counter=Decimal(52), aft_gen_do_counter=Decimal(52)))
    await db.flush()

    await recompute_leg(db, 1)
    evs = list((await db.execute(
        MRVEvent.__table__.select().order_by(MRVEvent.__table__.c.recorded_at)
    )).fetchall())
    e2 = evs[1]
...
async def test_event_edit_and_delete(db, staff_user):
    ...
    resp = await add_event(
        1, _Req(form={"event_kind": "noon_consumption", "recorded_at": "2026-04-02T12:00:00",
                      "fuel_mass_t": "5.5", "distance_nm": "120"}),
...
async def test_dnv_export_18_columns_with_imo(db, staff_user):
    ...
    db.add(_ev(1, datetime(2026, 4, 2, 12, tzinfo=UTC), fuel_mass_t=Decimal("5.5"),
               distance_nm=Decimal("120"), lat_deg=49, lat_min=Decimal("30.5"), lat_ns="N"))
    await db.flush()
    await recompute_leg(db, 1)
...
async def test_carbon_pdf_blocked_on_quality_error(db, staff_user):
    ...
    db.add(_ev(1, t0, port_me_do_counter=Decimal(100), stbd_me_do_counter=Decimal(100),
               fwd_gen_do_counter=Decimal(50), aft_gen_do_counter=Decimal(50)))
    db.add(_ev(1, t0 + timedelta(days=1), port_me_do_counter=Decimal(95),
               stbd_me_do_counter=Decimal(105), fwd_gen_do_counter=Decimal(52),
...
async def test_carbon_pdf_guard_is_scoped_to_vessel(db, staff_user):
    ...
    db.add(_ev(1, t0, port_me_do_counter=Decimal(100), stbd_me_do_counter=Decimal(100),
               fwd_gen_do_counter=Decimal(50), aft_gen_do_counter=Decimal(50)))
    db.add(_ev(1, t0 + timedelta(days=1), port_me_do_counter=Decimal(90),
               stbd_me_do_counter=Decimal(100), fwd_gen_do_counter=Decimal(50),
...
async def test_params_save_updates_density(db, staff_user):
    ...
    resp = await mrv_params_save(
        _Req(), avg_mdo_density="0.900", mdo_admissible_deviation="1.5",
        db=db, user=staff_user,
    )
    assert resp.status_code == 303
