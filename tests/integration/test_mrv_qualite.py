...
async def _leg(db, vessel, code="1AFRBR6"):
    ...
    leg = Leg(leg_code=code, vessel_id=vessel.id,
              ...
              etd_ref=T0, eta_ref=T0 + timedelta(days=5),
              etd=T0, eta=T0 + timedelta(days=5))
    db.add(leg)
    await db.flush()
    return leg
...
async def _bunker(db, vessel, bdn="BDN-24"):
    return await bunkering.create_draft(
        db, vessel=vessel, author_user_id=1, bdn_number=bdn, port_locode="FRFEC",
        delivery_datetime_utc=T0, mass_t=Decimal("20"),
...
async def _notifs(db, role):
    return list(
        (
            await db.execute(
                select(Notification).where(Notification.target_role == role)
            )
        ).scalars().all()
    )
...
async def test_r14_critical_routes_manager_and_admin(db, staff_user):
    ...
    dep = DepartureEvent(leg_id=leg.id, vessel_id=vessel.id, status="finalise",
                         datetime_utc=T0, rob_t=Decimal("100"), vessel_condition="laden")
    ...
    arr = ArrivalEvent(leg_id=leg.id, vessel_id=vessel.id, status="finalise",
                       ...
                       rob_t=Decimal("90"), vessel_condition="laden")  # écart ≈ 9,2 t
    arr.engine_readings = [NavEventEngineReading(engine_id=engines["PME"].id,
                                                 fuel_counter_l=Decimal("11000"))]
    db.add_all([dep, arr])
    await db.flush()

    summary = await run_rules(db, "voyage", [leg], vessel=vessel, leg=leg,
...
async def test_qualite_screen_renders_with_filters_and_counters(db, staff_user):
    ...
    resp = await mrv_qualite(
        FakeRequest(), rule=None, severity=None, vessel_id=None, leg_id=None,
        result=None, date_from=None, date_to=None, db=db, user=staff_user,
    )
    ...
    resp_r24 = await mrv_qualite(
        FakeRequest(), rule="R24", severity=None, vessel_id=None, leg_id=None,
        result="fail", date_from=None, date_to=None, db=db, user=staff_user,
    )
    ...
    resp_blk = await mrv_qualite(
        FakeRequest(), rule="R24", severity="bloquant", vessel_id=None, leg_id=None,
        result=None, date_from=None, date_to=None, db=db, user=staff_user,
    )
    ...
    resp_future = await mrv_qualite(
        FakeRequest(), rule=None, severity=None, vessel_id=None, leg_id=None,
        result=None, date_from="2030-01-01", date_to=None, db=db, user=staff_user,
    )
...
async def _regressed_chain(db, *, flagged_by_master=True):
    ...
    arr = ArrivalEvent(leg_id=leg.id, vessel_id=vessel.id, status="finalise",
                       ...
                       rob_t=Decimal("99"), vessel_condition="laden")
    arr.engine_readings = [NavEventEngineReading(
        engine_id=engines["PME"].id, fuel_counter_l=Decimal("900"),
...
async def test_confirm_reset_traces_and_makes_r10_pass(db, staff_user):
    ...
    s1 = await run_rules(db, "event", [dep, arr], vessel=vessel, leg=leg,
                         persist_passes=False)
    ...
    s2 = await run_rules(db, "event", [dep, arr], vessel=vessel, leg=leg,
                         persist_passes=False)
...
async def test_acknowledge_flow_and_errors(db, staff_user):
    ...
    ok = QualityCheckResult(rule_id="R24", subject_type="bunker_operations",
                            subject_id=bunker.id, run_id="x", result="pass")
    db.add(ok)
    await db.flush()
    with pytest.raises(HTTPException) as exc:
...
async def test_report_validate_master_triggers_r22(db, staff_user):
    ...
    vessel = await _vessel(db)
    leg = await _leg(db, vessel)
    carbon = EnvReport(leg_id=leg.id, report_type="carbon", status="brouillon",
                       payload={"totals": {"conso_total_t": "10"}})
    noon = EnvReport(leg_id=leg.id, report_type="noon", status="valide_master",
    ...
    r22 = (
        await db.execute(
            select(QualityCheckResult).where(QualityCheckResult.rule_id == "R22",
                                             QualityCheckResult.result == "fail")
        )
    ).scalars().all()
...
async def test_flgo_import_route_triggers_r25(db):
    ...
    ws.append([None, None, "", "Operation date", "14 - GO DB B", "15 - GO DB T",
               "Total volume [m3]", "ROB [m3]", "Remarks", "Docs"])
    ws.append([None, None, "Measurement", "06/07/2026 22:13",
               "14.6 m3 (12.76 t)", "10.4 m3 (9.03 t)", "31", "31", "", "0"])
    buf = io.BytesIO()
    wb.save(buf)

    user = SimpleNamespace(id=1, full_name="MRV", username="mrv", role="operation")
    resp = await mrv_flgo_import(
        FakeRequest(), vessel_id=vessel.id,
    ...
    r25 = (
        await db.execute(
            select(QualityCheckResult).where(QualityCheckResult.rule_id == "R25",
                                             QualityCheckResult.result == "fail")
        )
    ).scalars().all()
...
async def test_quality_run_cron_ok_scans_active_legs(db, monkeypatch, staff_user):
    ...
    db.add(p3)
    await db.flush()
    closed = Leg(leg_code="1BBRVN6", vessel_id=vessel.id,
                 departure_port_id=leg.arrival_port_id, arrival_port_id=p3.id,
                 etd_ref=T0 + timedelta(days=10), eta_ref=T0 + timedelta(days=20),
                 etd=T0 + timedelta(days=10), eta=T0 + timedelta(days=20),
